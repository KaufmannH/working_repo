...
#heatmap
pdf(file = paste0(out_folder,"/predictors/Heatmap_predictors_clusters_3.pdf"), width = 5, height = 4)
...
for (pred in c("TATA_box", "Inr", "CCAAT_box", "GC_box", 'count')) {
  ...
  ggsave(boxplot_list[[pred]], filename = file.path(out_folder, paste0('/predictors/', pred, "_boxplot_by_var.png")), 
    width = 4, height = 3)
}
...
ggsave(p, filename = file.path(out_folder, "/violin_var_med_gmean.png"), 
    width = 3, height = 3)


#number of genes per var_group barplot
...
ggsave(p, filename = paste0(out_folder, '/gene_count_per_var.png'), height = 3, width = 3)
...
ggsave(p, filename = file.path(out_folder, "/violin_var_downsample.png"), 
    width = 3, height = 3)
...
p<-ggplot(count_df, aes(x = var, y = count, fill = var)) +
        ...
        theme_classic() +
        theme(axis.text.x = element_text(angle = 10, hjust = 1), legend.position = "none")
ggsave(p, filename = paste0(out_folder, '/gene_count_per_var_downsample.png'), height = 3, width = 3)
...
write_tsv(prediction_data_matched, file = 'data_prep_for_classification/hepatocyte_models/results/hvg_lvg_predictors18m.tsv')
