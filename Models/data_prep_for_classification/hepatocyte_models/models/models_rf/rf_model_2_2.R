...
# predicts: LVG/HVG across all ages (just based on TF binding profile)
# input data: includes tf binding predictions

df_matched <- readRDS('data_prep_for_classification/hepatocyte_models/data/matched_tf_binding_df.rds')  
head(df_matched)
# select cols i need
data <- df_matched %>% 
  select(-c(cluster_id, age, res_var, gmean, median_gmean, diff, variability, TATA_box, Inr, CCAAT_box, GC_box, count)) %>%  
...
# normal fit
familial_cols <- grep("^Familial_profile", names(data), value = TRUE)

rf_fit <- rf_model %>% 
          fit(as.formula(paste("var_non_amb ~", paste(familial_cols, collapse = " + "))),
          data = train_data)
...
roc_plot <- ggplot(roc_data, aes(x = 1 - specificity, y = sensitivity)) +
  ...
  labs(
    title = sprintf("HVG ident. by TF binding pred.: AUC = %.3f", roc_auc_val),
    x = "Specificity (TP)",
    y = "Sensitivity (FP)") +
  theme_classic(base_size = 14) +
  theme(plot.title = element_text(size = 14, hjust = 0.5))

ggsave("data_prep_for_classification/hepatocyte_models/results/predictions/roc_rf_M2_2.png", roc_plot, width = 5, height = 5)
