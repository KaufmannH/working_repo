...
combine_technologies_2 <- function(){
  ...
  df_facs_raw <- read_csv("ImmuneNoise/facs/data/combined_data.csv")
  df_droplet_raw <- read_csv("ImmuneNoise/droplet/data/combined_data.csv") 
  df_pansci_raw <- read_csv("ImmuneNoise/pansci/data/combined_data.csv") 
  ...
  write_tsv(bound_df, "TMS/comparison/data/hep_all_tech_df.tsv")
...
# the lvgs could be genes that have expression 0 -> test correlation without them

test <- bound_df |>
  filter(gene %in% genes_shared_list) |>
  filter(lvg == TRUE) |>
  mutate(bigger_0 = (!gmean == 0))|>
  group_by(bigger_0) |>
  count()
test
...
gene_cluster_stats_non_0 <- bound_df |>
  mutate(stable_hvg = perc_hvg > 0.9) |>
  mutate(lvg = lvg & (!gmean == 0)) |>  # threw out 0 gmean lvgs
  distinct(gene, technology, cluster_id, hvg, lvg, stable_hvg) |>
  group_by(gene, technology) |>
  summarise(
    n_clusters_total = n_distinct(cluster_id),
