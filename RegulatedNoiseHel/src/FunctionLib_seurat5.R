...
res_var_subset <- function(so, cells_filter, count_slot_name, vst_out = NULL, integrated = FALSE){
  ...
  #cells_filter <- so@meta.data[[meta_col]] == '18'
  # raw counts for subset
  #umi.subset <- so@assays[[count_slot_name]]@counts[,cells_filter]
  umi.subset <- GetAssayData(object = so,
                              assay  = 'originalexp',
                              slot   = "counts" )[,cells_filter]

  # vst.out for subset
  if (is.null(vst_out)) {
    vst_out.subset           <- so@assays$SCT@misc$vst.out
    subset.cells             <- colnames(so)[cells_filter]
    vst_out.subset$cell_attr <- vst_out.subset$cell_attr[subset.cells,]
  } else {
