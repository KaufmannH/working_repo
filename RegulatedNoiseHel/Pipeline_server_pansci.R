...
Sys.setenv(RETICULATE_PYTHON = "/home/hkaufm49/anaconda3/envs/pansci_env/bin/python")
library(reticulate)
...
#for (tissue in SelectedTissues) { # generalize tissue to input file (with different possible input file formats?)
  ...
  # raw data folder
  so.path <- file.path("/home/hkaufm49/analyses/working_repo/PanSci/data/prepped_for_pipeline")
  files <- list.files(so.path, pattern = "\\.rds$", full.names = TRUE)
