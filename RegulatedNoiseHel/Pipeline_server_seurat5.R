...
source('src/FunctionLib_seurat5.R')
...
out_folder <- "data/processed/added_seurat5"          
...
#for (tissue in SelectedTissues) { # generalize tissue to input file (with different possible input file formats?)
  ...
  for (age_group in AgeSexGroups) {
    ...
    # 2) create the SCT assay
    so[["SCT"]] <- Seurat::CreateAssayObject(
    data   = vst_out$y)
    DefaultAssay(so) <- "SCT"
    ...
    # was so@assays$SCT@counts
    doublet_results <- detect_doublets(so@assays$originalexp@counts, so@meta.data$seurat_clusters)
